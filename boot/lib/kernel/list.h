...
//原理是用elem_ptr的地址减去elem_ptr在结构体struct_type中的偏移量，此地址差便是结构体struct_type的起始地址，最后再将此地址差转换为struct_type指针类型

/*定义链接节点成员结构，链表中不需要数据成员，只需要前驱和后继结点指针*/
struct list_elem{   //list_elem的地址是链表中的结点，但结点从链表中脱离出的时候，要将它还原成宿主数据结构才能使用
